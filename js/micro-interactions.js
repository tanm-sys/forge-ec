...
class MicroInteractionSystem {
  ...
  setupHoverEffects() {
    ...
    // Tilt hover effect for feature cards
    this.setupTiltHover();
  }
  ...
  setupTiltHover() {
    const tiltElements = document.querySelectorAll('.tilt-hover, .feature-card');
    ...
    tiltElements.forEach(element => {
      ...
      const handleMouseMove = (e) => {
        ...
        const animation = window.popmotion.animate({
          from: {
            rotateX: parseFloat(element.style.transform?.match(/rotateX\(([^)]+)\)/)?.[1] || 0),
            rotateY: parseFloat(element.style.transform?.match(/rotateY\(([^)]+)\)/)?.[1] || 0)
          },
          to: { rotateX, rotateY },
          duration: 200,
          ease: window.popmotion.easeOut,
  ...
  setupScrollEffects() {
    // Parallax scroll effects using Popmotion
    const parallaxElements = document.querySelectorAll('.parallax-smooth');
    
    if (window.smoothScrollSystem) {
      window.smoothScrollSystem.addScrollListener('micro-interactions', (data) => {
        this.updateParallaxElements(parallaxElements, data.scroll || window.scrollY);
      });
    }
  }

  updateParallaxElements(elements, scrollY) {
    if (this.isReducedMotion) return;
    ...
    elements.forEach(element => {
      ...
      window.popmotion.animate({
        from: parseFloat(element.style.transform?.match(/translateY\(([^)]+)\)/)?.[1] || 0),
        to: yPos,
        duration: 100,
        ease: window.popmotion.linear,
        onUpdate: (y) => {
  ...
  setupFormInteractions() {
    ...
    formInputs.forEach(input => {
      ...
      const handleBlur = () => {
        if (this.isReducedMotion || input.value) return;
