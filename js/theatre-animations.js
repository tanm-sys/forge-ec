...
class TheatreAnimationSystem {
  ...
  async loadTheatreLibraries() {
    ...
    try {
      ...
      // Load Theatre.js studio (development only)
      if (process.env.NODE_ENV === 'development') {
        const studioScript = document.createElement('script');
        studioScript.src = 'https://unpkg.com/@theatre/studio@0.5.1/dist/index.js';
        studioScript.type = 'module';
        
        await new Promise((resolve, reject) => {
          studioScript.onload = resolve;
          studioScript.onerror = reject;
          document.head.appendChild(studioScript);
        });
      }
  ...
  initializeProject() {
    ...
    if (process.env.NODE_ENV === 'development' && window.studio) {
      window.studio.initialize();
    }
  }
  ...
  saveProjectState() {
    if (this.project) {
      const state = this.project.getState();
      localStorage.setItem('theatre-forge-ec-state', JSON.stringify(state));
    }
  }
  ...
  createHeroAnimations() {
    ...
    if (heroSubtitle) {
      const subtitleObject = heroSheet.object('Hero Subtitle', {
        opacity: 0,
        y: 30,
        blur: 5
      });
      
      subtitleObject.onValuesChange((values) => {
        if (this.isReducedMotion) return;
        
        heroSubtitle.style.opacity = values.opacity;
        heroSubtitle.style.transform = `translateY(${values.y}px)`;
        heroSubtitle.style.filter = `blur(${values.blur}px)`;
      });
    }
  }
  ...
  createNavigationAnimations() {
    const navSheet = this.project.sheet('Navigation');
    this.sheets.set('navigation', navSheet);
    
    const navbar = document.getElementById('navbar');
    if (navbar) {
      const navObject = navSheet.object('Navbar', {
        y: -100,
        opacity: 0,
        backdropBlur: 0
      });
      
      navObject.onValuesChange((values) => {
        if (this.isReducedMotion) return;
        
        navbar.style.transform = `translateY(${values.y}px)`;
        navbar.style.opacity = values.opacity;
        navbar.style.backdropFilter = `blur(${values.backdropBlur}px)`;
      });
    }
  }

  // Animation playback methods
  async playHeroSequence() {
    if (!this.isInitialized || this.isReducedMotion) return;
    
    const heroSheet = this.sheets.get('hero');
    if (!heroSheet) return;
    
    try {
      this.markPerformanceStart('hero-animation');
      
      // Play hero animation sequence
      await heroSheet.sequence.play({
        range: [0, 3],
        rate: 1,
        direction: 'normal'
      });
      
      this.markPerformanceEnd('hero-animation');
      
    } catch (error) {
      console.warn('⚠️ Hero animation failed:', error);
    }
  }

  async playFeatureSequence() {
    if (!this.isInitialized || this.isReducedMotion) return;
    
    const featureSheet = this.sheets.get('features');
    if (!featureSheet) return;
    
    try {
      this.markPerformanceStart('feature-animation');
      
      await featureSheet.sequence.play({
        range: [0, 2],
        rate: 1.2,
        direction: 'normal'
      });
      
      this.markPerformanceEnd('feature-animation');
      
    } catch (error) {
      console.warn('⚠️ Feature animation failed:', error);
    }
  }
  ...
  markPerformanceStart(name) {
    if (this.performanceMonitor) {
      this.performanceMonitor.mark(`${name}-start`);
    }
  }

  markPerformanceEnd(name) {
    if (this.performanceMonitor) {
      this.performanceMonitor.mark(`${name}-end`);
      this.performanceMonitor.measure(name, `${name}-start`, `${name}-end`);
    }
  }
  ...
  optimizeAnimations() {
    ...
    this.sheets.forEach(sheet => {
      if (sheet.sequence) {
        ...
        setTimeout(() => sheet.sequence.play(), 100);
      }
    });
  }
